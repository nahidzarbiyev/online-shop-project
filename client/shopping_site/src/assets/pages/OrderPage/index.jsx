    ...
    <>
      ...
      <div className="flex flex-col  justify-between gap-10  my-10 w-full lg:max-w-[600px] max-w-[400px] mx-auto ">
        ...
        {user.orders.map((order) => {
          return order.items.map((elem) => {
            return (
              <Link
                to={`/orderDetails/${order._id}`}
                key={elem._id}
                className="flex flex-col justify-between bg-secondary items-end lg:flex-row gap-5"
              >
                ...
                <p className="text-green-600 p-3 uppercase text-sm font-bold">
                  {" "}
                  {order.paymentStatus}
                </p>
              </Link>
